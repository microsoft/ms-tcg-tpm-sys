...
// important.
static PLATFORM: LazyLock<Mutex<Option<MsTpm20RefPlatformImpl>>> =
...
impl MsTpm20RefPlatform {
    ...
    pub fn initialize(
        ...
        match &mut *maybe_platform {
            ...
            None => {
                let platform = match &init_kind {
                    InitKind::ColdInit => {
                        let mut platform =
                            MsTpm20RefPlatformImpl::new(callbacks, api::nvmem::NV_MEMORY_SIZE);
                        platform.nv_enable()?;
                        platform
                    }
        ...
        if matches!(
            &init_kind,
            InitKind::ColdInit | InitKind::ColdInitWithSize(_)
        ) {
    ...
    pub unsafe fn execute_command_unchecked(
        ...
        if prev_response_ptr != response_ptr {
            ...
            tracing::warn!(
                "TPM library returned a response ptr that doesn't match the provided response buffer: {:#x?} != {:#x?}",
                prev_response_ptr,
                response_ptr
            );
